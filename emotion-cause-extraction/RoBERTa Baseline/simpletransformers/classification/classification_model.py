    ...
    def predict(self, to_predict, multi_label=False):
        ...
        if not multi_label and self.args.onnx:
            ...
            if self.config.output_hidden_states:
                ...
                preds = None
                out_label_ids = None
                for i, batch in enumerate(tqdm(eval_dataloader, disable=args.silent, desc="Running Prediction", position=0, leave=True)):
                    # batch = tuple(t.to(device) for t in batch)
                    with torch.no_grad():
                        inputs = self._get_inputs_dict(batch)
                ...
                n_batches = len(eval_dataloader)
                for i, batch in enumerate(tqdm(eval_dataloader, disable=args.silent, position=0, leave=True)):
                    model.eval()
                    # batch = tuple(t.to(device) for t in batch)
